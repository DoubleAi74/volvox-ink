...
//   // 1. Find the page to delete to get its metadata and order_index
//   const pageToDelete = allPages.find((p) => p.id === pageId);
//   if (!pageToDelete) {
//     console.error("Could not find page to delete in the provided list.");
...
//   // 2. Fetch all posts that belong to this page
//   const postsToDelete = await getPostsForPage(pageId);

//   // 3. Gather all file URLs from the page's thumbnail and its posts
//   const urlsToDelete = [];
//   if (pageToDelete.thumbnailUrl) {
//     urlsToDelete.push(pageToDelete.thumbnailUrl);
//   }
//   postsToDelete.forEach((post) => {
//     if (post.fileUrl) urlsToDelete.push(post.fileUrl);
//     if (post.thumbnailUrl) urlsToDelete.push(post.thumbnailUrl);
//   });

//   // 4. Delete all collected files from Storage in parallel
//   const deleteFilePromises = urlsToDelete.map((url) => {
...
//   // 5. Add all child posts to the delete batch
//   postsToDelete.forEach((post) => {
//     const postRef = doc(db, "posts", post.id);
//     batch.delete(postRef);
//   });

//   // 6. Update the order_index for all subsequent pages
//   allPages.forEach((page) => {
//     if (page.order_index > deletedIndex) {
//       const pageRef = doc(db, "pages", page.id);
//       batch.update(pageRef, { order_index: page.order_index - 1 });
...
//   // 7. Add the main page document to the delete batch
//   const pageToDeleteRef = doc(db, "pages", pageId);
//   batch.delete(pageToDeleteRef);

//   // 8. Commit all Firestore operations at once
//   await batch.commit();
// };
...
//   if (!postToDelete) {
//     console.error("Could not find post to delete in the provided list.");
//     // Fallback to just delete the Firestore document if post data is not available
//     await deleteDoc(doc(db, "posts", postId));
//     return;
//   }
...
//   const deleteFileFromStorage = async (url) => {
//     if (!url) return; // Do nothing if the URL is empty
//     try {
//       const fileRef = ref(storage, url); // Get storage reference from download URL
//       await deleteObject(fileRef);
//     } catch (error) {
//       // Log the error but don't stop the process.
//       // The file might have been deleted manually or the URL might be invalid.
//       console.error("Failed to delete file from storage:", error);
//     }
//   };

//   // Delete the main file and the thumbnail from storage
//   await deleteFileFromStorage(postToDelete.fileUrl);
//   await deleteFileFromStorage(postToDelete.thumbnailUrl);
//   // --- END: File Deletion Logic ---

//   // --- EXISTING: Firestore Deletion Logic ---
//   const batch = writeBatch(db);
//   const deletedIndex = postToDelete.order_index;
...
//   // Delete the target post document from Firestore
//   const postToDeleteRef = doc(db, "posts", postId);
//   batch.delete(postToDeleteRef);
...
////\/\/\//\/\/\\//\/
// export const deletePost = async (postId, allPosts) => {
//   console.log("--- Starting deletePost operation ---");

//   const postToDelete = allPosts.find((p) => p.id === postId);

//   if (!postToDelete) {
//     console.error(
//       "CRITICAL: Could not find post to delete in the provided list. Aborting."
//     );
//     // Fallback to just delete the Firestore document if post data is not available
//     await deleteDoc(doc(db, "posts", postId));
//     return;
...
//   // --- DIAGNOSTIC LOGS ---
//   console.log("Found post object to delete:", postToDelete);
//   console.log("File URL to be deleted:", postToDelete.fileUrl);
//   console.log("Thumbnail URL to be deleted:", postToDelete.thumbnailUrl);
//   // --- END DIAGNOSTIC LOGS ---

//   // --- NEW: File Deletion Logic with enhanced logging ---
//   const deleteFileFromStorage = async (url) => {
//     // Do nothing if the URL is empty, null, or undefined
//     if (!url) {
//       console.log("Skipping deletion because URL is empty.");
//       return;
//     }

//     try {
//       console.log(`Attempting to create reference for: ${url}`);
//       const fileRef = ref(storage, url); // Get storage reference from download URL

//       console.log(
//         `Reference created. Attempting to delete object at path: ${fileRef.fullPath}`
//       );
//       await deleteObject(fileRef);

//       // If we get here, the delete command was sent without an error
//       console.log(
//         `SUCCESS: deleteObject call for ${fileRef.fullPath} completed.`
//       );
//     } catch (error) {
//       // If Firebase returns an error (like permission denied), it will be caught here.
//       console.error(
//         `ERROR: Failed to delete file from storage. URL was: ${url}`
//       );
//       console.error("The full error object is:", error);
//       console.error("Firebase error code:", error.code); // This is very important!
//     }
//   };
...
//   console.log("Proceeding with Firestore database deletion...");
//   // --- EXISTING: Firestore Deletion Logic ---
//   const batch = writeBatch(db);
//   const deletedIndex = postToDelete.order_index;
...
//   await batch.commit();
//   console.log("--- Finished deletePost operation ---");
// };
